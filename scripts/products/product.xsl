...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:bodyview="http://www.navitas.nl/2014/Mapper/dbaccess/bodyview" xmlns:user="http://www.navitas.nl/2014/Mapper/scripts" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:m="http://www.navitas.nl/2014/Mapper">
  <xsl:template match="/">
    <target>
      <bodyview3>
        <xsl:for-each select="bodyview:query('select p.*, ws.title, ws.metakeywords, ws.metadescription from str_product p left join cms_websiteitems ws on ws.productid = p.id and ws.websiteid = 6 where not p.deleted and p.categoryid != 0 and (p.parentid is null or p.parentid in (select id from str_product where not deleted))')">
          <product>
