...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:bodyview="http://www.navitas.nl/2014/Mapper/dbaccess/bodyview" xmlns:m="http://www.navitas.nl/2014/Mapper" xmlns:user="http://www.navitas.nl/2014/Mapper/scripts" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
    ...
    <target>
      <bodyview3>
        <xsl:for-each select="bodyview:query(user:query())">
          <templatedetails>
            <templatedetailsid m:left="44" m:top="82">pk:templatedetails(mail-<xsl:value-of select="id" />)</templatedetailsid>
            <xsl:if test="businessid != 0">
              <businessid m:left="-87" m:top="152">
                <xsl:value-of select="businessid" />
              </businessid>
            </xsl:if>
            <templateid m:left="39" m:top="169">fk:template(mail -<xsl:value-of select="templatename" />)</templateid>
  ...
  <msxsl:script implements-prefix="user" language="CSharp"><![CDATA[
    public string query()
    {
        return @"select t.templatename, td.* from sys_mailtemplatedetails td 
                inner join sys_mailtemplate t on t.id = td.templateid
                where t.templatename not in ('Order confirmation', 'Order success') or businessid != 3";
    }
    ]]></msxsl:script>
</xsl:stylesheet>