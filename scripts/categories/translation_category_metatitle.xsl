...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:m="http://www.navitas.nl/2014/Mapper" xmlns:bodyview="http://www.navitas.nl/2014/Mapper/dbaccess/bodyview">
  <xsl:template match="/">
    <target>
      <bodyview3>
        <xsl:for-each select="bodyview:query('select wi.id, w.businessid, wi.languageid, wi.categoryid, wi.title from cms_websiteitems wi inner join cms_website w on w.websiteid = wi.websiteid where wi.title is not null and categoryid is not null and w.businessid = 3')">
          <translation>
