...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:bodyview="http://www.navitas.nl/2014/Mapper/dbaccess/bodyview" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:user="http://www.navitas.nl/2014/Mapper/scripts" xmlns:m="http://www.navitas.nl/2014/Mapper">
  <xsl:template match="/">
    <target>
      <bodyview3>
        <xsl:for-each select="bodyview:query('select c.id, w.imageid, f.filename, f.contenttype from str_category c inner join cms_websiteitems w on w.categoryid = c.id  inner join str_files f on f.id = w.imageid where imageid is not null  and w.languageid = 1 and w.websiteid = 6')">
          <file>
            <fileid m:left="-372" m:top="65"><xsl:value-of select="imageid" /></fileid>
            <filetypeid m:left="4" m:top="171">2</filetypeid>
            ...
            <languageid m:left="-50" m:top="132">1</languageid>
