...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:mrep="http://www.navitas.nl/2014/Mapper/dbaccess/mrep" xmlns:m="http://www.navitas.nl/2014/Mapper" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:user="http://www.navitas.nl/2014/Mapper/scripts">
  <xsl:template match="/">
    <target>
      <mrep3>
        <xsl:for-each select="mrep:query('select distinct m.*, s.paymentgatewaytypeid as paymentgatewayid, c.businessid, pbc.countryid from str_paymentwebtype m inner join str_paymentmethodecost c on c.paymentmethodid = m.id inner join str_paymentwebtypebycountry pbc on pbc.paymentgatewayid = c.paymentgatewayid and pbc.businessid = c.businessid left join str_paymentsettings s on s.paymentgatewaytypeid = c.paymentgatewayid and s.businessid = c.businessid')">
          <product>
